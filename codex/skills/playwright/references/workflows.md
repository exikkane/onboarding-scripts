...
Use the wrapper script and snapshot often. In projects based on `docker-sample`, use `bash scripts/pwcli.sh ...` from the repository root. For shorthand examples below, `pwcli` means `bash scripts/pwcli.sh` when that file exists; otherwise it means the normal skill wrapper alias. Project-local runs keep artifacts and caches inside ignored project paths.
